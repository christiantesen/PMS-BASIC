...
// Projects routes
app.get('/api/projects', auth, asyncHandler(async (req: any, res) => {
...
app.post('/api/projects', auth, checkRole(['ADMIN', 'MANAGER']), asyncHandler(async (req, res) => {
...
// Tasks routes
app.get('/api/projects/:projectId/tasks', auth, asyncHandler(async (req: any, res) => {
  const { projectId } = req.params;
  const tasksList = await db.select().from(tasks).where(eq(tasks.projectId, parseInt(projectId)));
  res.json(tasksList);
}));

app.post('/api/projects/:projectId/tasks', auth, checkRole(['ADMIN', 'MANAGER']), asyncHandler(async (req, res) => {
  const { projectId } = req.params;
  ...
  if (assigneeId) {
    const user_data = await db.select().from(users).where(eq(users.id, assigneeId));
...
app.post('/api/tasks/:taskId/assign', auth, checkRole(['ADMIN', 'MANAGER']), asyncHandler(async (req, res) => {
...
app.post('/api/tasks/:taskId/status', auth, asyncHandler(async (req: any, res) => {
  const { taskId } = req.params;
  const { status, comment } = req.body;

  const [currentTask] = await db.select().from(tasks).where(eq(tasks.id, parseInt(taskId)));
...
app.post('/api/tasks/:taskId/abandon', auth, asyncHandler(async (req: any, res) => {
  const { taskId } = req.params;
...
// Task history routes
app.get('/api/tasks/:taskId/history', auth, asyncHandler(async (req, res) => {
  const { taskId } = req.params;
